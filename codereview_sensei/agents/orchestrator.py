...
class OrchestratorAgent:
    ...
    async def run_all(self, files: list[PRFile]) -> dict:
        """Executes all sub-agents in parallel and prints progress."""
        async def run_bh():
            res = await self.bug_hunter.review(files)
            print(f"✅ Bug Hunter complete — {len(res)} findings")
            return res

        async def run_sa():
            res = await self.security_auditor.review(files)
            print(f"✅ Security Auditor complete — {len(res)} findings")
            return res

        async def run_sr():
            res = await self.style_reviewer.review(files)
            print(f"✅ Style Reviewer complete — {len(res)} findings")
            return res

        async def run_sl():
            res = await self.style_learner.review(files)
